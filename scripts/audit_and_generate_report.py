...
def make_table(rows):
    headers = ['方案 / 变体', 'AUROC ↑', 'Rej-F1@0.4 ↑', 'mAP ↑', 'mR@5 ↑', 'mR+@5 ↑', 'G-mIoU@3 ↑', '可追溯 JSON 文件', '可追溯日志文件']
    md = '| ' + ' | '.join(headers) + ' |\n'
    md += '| ' + ' | '.join(['---'] * len(headers)) + ' |\n'
...
# Table 1: Moment Main
t1 = make_table([
    ('Moment-DETR Strict GMR (Base)', 'artifacts/formal_strict/moment_detr/seed2023/md_gmr_b128/best_joint_soccer_gmr_val_preds_metrics.json', 'artifacts/formal_strict/moment_detr/seed2023/md_gmr_b128/stdout.log'),
    ('Moment HieA2M Parent', 'artifacts/formal_strict/moment_detr/seed2023/md_hiea2m_b128_rerun_from_best_v2/best_joint_soccer_gmr_val_preds_metrics.json', 'artifacts/formal_strict/moment_detr/seed2023/md_hiea2m_b128_rerun_from_best_v2/stdout.log'),
...
t2 = make_table([
    ...
    ('+ Dual Gate (EaTR Dual)', 'artifacts/eatr_dgqc_transfer/seed2023/eatr_dual/best_joint_val_metrics.json', 'artifacts/eatr_dgqc_transfer/seed2023/eatr_dual/stdout.log'),
...
t3 = make_table([
    ...
    ('Flash-VTG GMR Release Anchor', 'artifacts/flash_vtg_supplement/release_gmr_val/strict_gmr_val_metrics_raw.json', 'artifacts/flash_vtg_supplement/release_gmr_val/eval.log.txt'),
...
# Table 7: Stage-B Cross-Backbone
t7 = make_table([
...
- **100% 可追溯性**：主表中所有 32 项主要实验结果均能够在 `/home/guoxiangyu/generalized-moment-retrieval/artifacts/` 目录下找到对应的 `.json` 预测评估文件和 `stdout.log` / `train_log.jsonl` 日志文件。
...
with open(os.path.join(repo_root, 'GMR_Progress_Report_Updated_20260723.md'), 'w', encoding='utf-8') as f:
    f.write(full_doc)

print("Successfully written audited markdown document!")
