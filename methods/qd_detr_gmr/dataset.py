"""Soccer-GMR feature dataset used by the isolated QD-DETR runner."""
...
class SoccerGMRDataset(Dataset):
    ...
    def _load_text(self, qid: int | str) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        valid = (
            np.asarray(attention_mask).reshape(-1) > 0
            if attention_mask is not None else np.ones(len(feature), dtype=bool)
        )
        if len(valid) != len(feature) or not valid.any():
            raise ValueError(f"invalid attention mask in {path}")
        if self.trim_text_by_attention_mask:
            feature = feature[valid]
            valid = np.ones(len(feature), dtype=bool)
        feature = feature[: self.max_q_l]
        valid = valid[: self.max_q_l]
        return (
            torch.from_numpy(_l2_normalize(feature)),
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        query, query_semantic_mask = self._load_text(meta["qid"])
        return {
            "meta": meta,
            "model_inputs": {
                "query_feat": query,
                "query_semantic_mask": query_semantic_mask,
                "video_feat": video,
...
def collate_fn(batch: Sequence[dict]) -> tuple[list[dict], dict]:
    query_semantic_mask, _ = _pad_1d([
        item["model_inputs"]["query_semantic_mask"] for item in batch
    ])
...
def prepare_batch(batch: tuple[list[dict], dict], device: torch.device) -> tuple[list[dict], dict, dict]:
    ...
    inputs = {
        ...
        "src_txt_semantic_mask": collated["query_semantic_mask"].to(device),
