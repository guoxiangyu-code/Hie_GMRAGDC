...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed, video_length=None):
        ...
        # Upstream used ``.cuda()`` here, which breaks CPU execution and
        # non-default CUDA devices.  Keep the tensor colocated with ``src``.
        tgt = torch.zeros(refpoint_embed.shape[0], bs, d, device=src.device, dtype=src.dtype)
