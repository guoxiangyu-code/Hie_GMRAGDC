...
class TemporalDualGrounding(nn.Module):
    ...
    def forward(
        ...
        # Mask only the residual. Preserving the parent's padded token values
        # makes the zero-gate warm start bitwise identical even for backbones
        # whose projection layers have non-zero bias; downstream EaTR masks
        # still exclude those padded tokens from attention.
        residual = sentence_gate * sentence_video + phrase_gate * phrase_video
        residual = residual * video_mask[..., None].to(residual.dtype)
        fused = video + residual
